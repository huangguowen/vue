<template>
    <a-card :bordered="false" style="position: relative">
        <a-col :span="9" style="position: static; overflow-y: auto;
    height: 500px;">
            <a-tree
                    v-if="orgTree.length"
                    :tree-data="orgTree"
                ...
                <a-form layout="inline">
                    <a-row :gutter="48">
                        ...
                        <span class="table-page-search-submitButtons cl">
                                <a-button v-if="hiddenMenu(a_menu)" icon="plus" style="margin-left: 5px" @click="handleAdd">添加</a-button>
                                <a-button v-if="hiddenMenu(d_menu)" icon="delete" style="margin-left: 5px" @click="handleDel">删除</a-button>
                                <a-button v-if="hiddenMenu(sort_menu)" icon="arrow-up" style="margin-left: 5px"
                                          @click="resetTreeUp('up')"></a-button>
                                <a-button v-if="hiddenMenu(sort_menu)" icon="arrow-down" style="margin-left: 5px"
                                          @click="resetTreeUp('down')"></a-button>
                            </span>
                    </a-row>
                </a-form>
            </div>
        ...
        <a-col :span="15">
            <div>
                <create-menu ref="menuModal"></create-menu>
            </div>
        </a-col>
...
<script>
    ...
    import {getPermissionList, del, sortUpdate, getFunctionList, functionAdd, sortExchange} from '@/api/framework/menu'
        ...
        methods: {
            ...
            handleAdd() {
                if (this.selectedKey != '' && this.selectedKey != "0-0") {
                    this.$refs.menuModal.addSon(this.selectedRowKeys)
                } else {
            ...
            handleDel() {
                ...
                if (this.selectedKey != '' && this.selectedKey != "0-0") {
                    this.$confirm({
                        ...
                        cancelText: '取消',
                        onOk: () => {
                            del(this.selectedRowKeys.id).then((res) => {
            ...
            /* 排序 */
            resetTreeUp(action) {
                sortUpdate({id: this.selectedRowKeys.id, action: action})
                    .then(res => {
                        if (res.data[0] == true) {//排序成功
                            this.selectedKey = []
                            this.initData()
                        }
            ...
            handleSelect(selectKey, info) {
                if (selectKey == '0-0') {
                    ...
                    if (info.selected) {
                        ...
                        this.display = true
                        this.$refs.menuModal.edit(this.selectedRowKeys)
                    } else {
                        this.selectedKey = []
                        this.selectedRowKeys = []
                        this.$refs.menuModal.add()
            ...
            onDrop(info) {
               ...
               // console.log(dragKey)
                ...
                sortExchange({dropId: dropMenuId, dragId: dragMenuId, to: to_dir, type: type, pos: dropPosition})
                    .then(res => {
                        console.log(res)
                        this.initData()
                        // 取消选中效果
